...
// Update an office user (including activate/deactivate).
app.put('/api/users/:id', authUser, requireRoles('ADMIN'), async (req, res) => {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'role')) {
      const cleanRole = String(body.role || '').trim().toUpperCase();
      if (!['ADMIN', 'MANAGER', 'SUPERVISOR'].includes(cleanRole)) {
        return res.status(400).json({ error: 'Valid role is required' });
      }
      updates.role = cleanRole;

      // Do not carry role-specific permission overrides into a new role.
      if (cleanRole !== String(user.role || '').trim().toUpperCase()) {
        updates.permissions = {};
      }
    }
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'isActive')) {
      ...
      if (Number(req.user.id) === userId && body.isActive === false) {
    ...
    // Keep at least one active administrator in the system.
    if (currentRole === 'ADMIN' && user.isActive && (nextRole !== 'ADMIN' || !nextIsActive)) {
...
app.post('/api/project-lifts/:projectLiftId/amc', async (req, res) => {
  try {
    ...
    if (!start) {
      return res.status(400).json({ error: 'Valid AMC start date is required' });
    }

    const months = Number(durationMonths || 12);
    const interval = Number(serviceIntervalDays || 90);
    const visits = Number(serviceVisitCount || 5);
    ...
    return res.status(500).json({ error: err.message || 'Failed to create AMC' });
  }
});
